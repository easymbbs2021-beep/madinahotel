...
const MetricCard: React.FC<{ title: string; value: string | number; icon: JSX.Element }> = ({ title, value, icon }) => (
    <div className="bg-white p-6 rounded-lg shadow-lg border border-gray-200 flex items-start">
...
const OwnerViewPage: React.FC = () => {
  ...
  return (
    <div className="container mx-auto py-16 px-4 sm:px-6 lg:px-8">
      <h1 className="text-5xl font-display text-gray-900">Owner's Dashboard</h1>
      <p className="text-gray-600 mt-2 mb-10">A real-time overview of Madina Hotel's performance.</p>
      ...
      <div className="bg-white p-6 rounded-lg shadow-xl border border-gray-200">
        <h2 className="text-3xl font-display mb-6 text-gray-900">Recent Orders</h2>
              ...
              {sortedOrders.map(order => (
                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <div className="font-medium text-gray-900 font-mono">{order.id}</div>
                      <div className="text-gray-500">{order.orderTime.toLocaleString()}</div>
                  </td>
